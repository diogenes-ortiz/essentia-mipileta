...
3. **"La familia Essentia completa"** — banner FOTO full-width (`familia-essentia-3.jpg`, 2480x1772 en alta: toda la línea con la rose al centro; versiones previas siguen en assets), con el **título adentro de la foto** (`.familia-title`, top 8%, **todo en el mismo peso**: la negrita de la segunda línea se sacó ago 2026 con `.intro-h2.familia-title strong{font-weight:inherit}`, calificado así para ganarle a `.intro-h2 strong{font-weight:700}`, que está más abajo) y el isologo chiquito arriba-izq. Sin crossfade (se probó alternar con `familia-foto.png` y el usuario lo descartó). Los videos `familia-*.mp4` ya no se usan.
4. **"Conocé cada detalle"** — arriba de los tabs va la **bajada** (`.detalle-lead`) que describe la línea entera: premium para cocina, cuatro modelos, **tres modos de colocación** y **tratamiento PVD**. Va afuera de los paneles a propósito, para que se lea sin tener que clickear (las colocaciones viven en otro tab). Después, barra de **3 opciones** (Modelos y colores · Modos de instalación · Desagüe), el panel se despliega debajo. El JS de los tabs es genérico (recorre todos los `.t3` y `.t3-panel`), así que sumar una opción es solo markup + pasar `.tabs3-bar` a 3 columnas:
...
5. **"Essentia desde adentro"** — *(ago 2026: se movió ACÁ, justo después de "Conocé cada
   ...
   - **Desagüe** (ago 2026): foto `desague-essentia.jpg` (1200x900, ya recortada a 4:3) dentro de `.inst-media`, una clase que tenía el CSS escrito pero había quedado sin uso (antes llevaba `assets/instalacion.jpg`). La foto es el **despiece apilado** (tapa + cestillo + cuerpo) que pasó el cliente, recortada 1:1 sobre la pieza. Va con `.inst-media--cuadrada` (marco 1:1, max-width 460px) porque la composición es vertical y en 4:3 quedaba chica y con mucho aire a los costados. Origen: `Downloads/ChatGPT Image Aug 24, 2026, 05_51_34 PM.png` (1254x1254). **VOCABULARIO (dato del cliente, ago 2026): la pieza del medio es el CESTILLO, no un "filtro".** Lo que hace es **sellar la pileta y dejarla hermética**, no colar. Está en el texto del panel con `<strong>` (que se resalta con `.t3-panel p strong`, porque un `strong` suelto resolvía a 400 sobre un párrafo de 300 y casi no se distinguía).
...
8. **CTA final** `#compra-segura` (enlazada desde el menú) "Compra segura" + bloque de **Garantía de 10 Años** (texto AISI 304-18/8 y 18/10). **La nota de Mercado Libre se sacó de acá (ago 2026)**: estaba repetida, la misma frase ya aparece en Complementos. Queda UNA sola `.ml-nota` en toda la página.
...
10. **"El mismo acero, en el baño"** (`#otra-linea`, ago 2026) — cierre con botón outline
    **"Conocé la línea Zíngara"** → `mipileta.com.ar/zingara` (ago 2026: antes iba a GitHub Pages, que tiene gate de contraseña; con Essentia ya pública eso mandaba visitantes a un muro).
