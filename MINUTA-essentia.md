...
2. **Hero ESSENTIA** (banner full-width):
   ...
   - **Mobile:** usa la MISMA foto (`essentia-hero-trio.png`) con `aspect-ratio:4/5` y `background-position:center` (entra la cuba rose entera), y **muestra el mismo wordmark** (width min(64vw,320px), top 6%). La vertical vieja `essentia-hero-mobile.png` quedó sin uso: traía el texto incrustado en baja calidad.
...
- **Botón "Descargar catálogo" alineado al desagüe:** el JS del final calcula dónde cae el tapón de la pileta rose sobre el render real del background (cover + center) y posiciona el botón ahí (constantes `FOTO_W/FOTO_H/TAPON_X=833.5`, medido sobre la foto: el tapón está en 49,85% del ancho, NO en el centro). Si se cambia la foto del hero, hay que volver a medir `TAPON_X`.
