...
## Secciones (en orden)
1. **Nav superior** (sticky negro): logo a la izquierda (48px desktop / 40 mobile), links **Concepto · Modelos · Compra segura · Contacto** (los targets llevan `scroll-margin-top:88px` para que el nav sticky no tape el título). En **mobile = menú hamburguesa**.
2. **Hero ESSENTIA** (banner full-width):
   ...
   - La foto va en `.hero::before` (para animarla sin mover el wordmark), con `background-size:cover`.
...
3. **"Diseño puro, funcionalidad absoluta"** + **VIDEO de producto** (`assets/video-essentia.mp4`, H.264 1920x1080, 1:56, **6,3 MB** — comprimido con ffmpeg desde 70 MB: `-an -crf 27 -preset medium -movflags +faststart`; sin audio porque va muted). Autoplay muted loop playsinline + `preload="auto"`, y un JS lo fuerza a reproducir apenas carga la página: **Chrome pausa los videos fuera de viewport**, por eso antes arrancaba de cero justo al llegar scrolleando. Reemplazó al modelo 3D (model-viewer + `670_Gris.glb` + JS de rotación por scroll, todo eliminado del HTML; el .glb sigue en assets). Ya no se carga el script CDN de model-viewer.
...
6. **Accesorios** — "Todo lo que necesitás, incluido" + **4 cards en una fila** (grid 4 col; 2x2 en ≤900px) con las **fotos oficiales** de `Downloads/Accesorios Web/` (PNG 3072px sin fondo): Barra escurridora (`acc-barra.png`), Tabla de vidrio (`acc-tabla.png`), Cesto escurridor (`acc-cesto.png`), Rejilla flexible (`acc-rejilla.png`) + botón Comprar en Mercado Libre.
   ...
   - **Variantes sin usar** en `Downloads/Accesorios Web/`: cesto en **rose** (Accesorio (3)), rejilla en **negro** (Accesorio (1)) y vistas alternativas de tabla (7) y barra (Accesorio.png). Sirven si se quiere crossfade acero/variante por card.
